...
metrics = pd.read_csv('results/topological_metrics/g_topological_metrics.csv')
...
cositas = metrics[['Gene', 'Gene name', 'Pathway', 'Essential']]
...
temp = metrics.loc[test_indices, ['Gene', 'Gene name', 'Pathway']]
gene_names = temp['Gene name'].values
gene_idx = temp['Gene'].values
pathways = temp['Pathway'].values
...
rf = RandomForestClassifier(n_estimators=300, class_weight=class_dict, random_state=42)
...
data['High confidence'] = indices
new_data = data[indices]

essentiality_ratio = new_data.groupby('Gene name')['Prediction'].apply(
    lambda x: x.sum() / len(x)
)

threshold = 0.4
genes_differing = essentiality_ratio[
    (essentiality_ratio >= 0.5 - threshold) & (essentiality_ratio <= 0.5 + threshold)]
cosa = genes_differing.sort_values(ascending=False)
top_20_names = cosa.index.tolist()
print(top_20_names)
...
pathways_20 = []
i = 0
for gene, pathway, pred, label in tqdm(zip(gene_idx, pathways, y_pred, y_test)):
    if label == 1:
        continue
    this_gene = new_data[new_data['Gene name'] == idx_to_name[int(float(gene))]]
    if gene in top_20 and pathway in this_gene['Pathway'].values:
...
gt.graph_draw(subgraph, pos,
              ...
              vertex_font_size=28,  # Adjust font size for readability
              vertex_text_color="black",  # Set text color to black
              vertex_size=6,
              edge_color=sub_edge_colors,
              output_size=(1000, 1000), output='results/aquihostia_g.png')
gt.graph_draw(subgraph, pos,
              vertex_fill_color=sub_vertex_colors,
              vertex_text=gene_pathway_labels,vertex_text_position=1,  # Adjust position if necessary
              vertex_font_size=28,  # Adjust font size for readability
              ...
              vertex_size=6,
              edge_color=sub_edge_colors,
              output_size=(1000, 1000), output='results/aquihostia_g.pdf')
